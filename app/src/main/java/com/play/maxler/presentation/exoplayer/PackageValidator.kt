...
class PackageValidator @Inject constructor(
    ...
    @MediaBrowseCallers @XmlRes xmlResId: Int
) {
    ...
    private fun logUnknownCaller(callerPackageInfo: CallerPackageInfo) {
        if (callerPackageInfo.signature != null) {
            Log.i("callers_errors","Package ${callerPackageInfo.name} with package name ${callerPackageInfo.packageName} and signature ${callerPackageInfo.signature} is unknown")
        }
    }
    ...
    private fun buildCertificateWhiteList(parser: XmlResourceParser): Map<String, KnownCallerInfo> {
        ...
        try {
            // at first equal --> <?xml version="1.0" encoding="utf-8"?>
            var eventType: Int = parser.next()
            // after next equal -->         <allowed_callers>
            // if not equal END_DOCUMENT -> </allowed_callers>
            while (eventType != XmlResourceParser.END_DOCUMENT) {
                // then equal -->           <allowed_callers>
            ...
            Log.i("callers_errors", "buildCertificateWhiteList: Could not read allowed callers from XML")
        } catch (e: IOException) {
            Log.i("callers_errors", "buildCertificateWhiteList: Could not read allowed callers from XML")
        }
        return certificates
    ...
    private fun parseTag(parser: XmlResourceParser): KnownCallerInfo? {
        ...
        while (eventType != XmlResourceParser.END_TAG) {
            ...
            val signature = parser.nextText().replace(Constants.WHITESPACE_REGEX, "").lowercase()
